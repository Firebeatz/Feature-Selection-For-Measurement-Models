...
# c)  back transformation of related dummy variables into one variable again for better interpretation of results 
...
# d) Implementation of success measure Suc, according to [BOLO-13]
...
# e) Implementation of Stability Measures, according to [NOGU-18]
...
# f) CFS filter implementation using FSelector from Romanski, P.; Kotthoff, L. (2018):
evaluator <- function(subset) {
  testedSet <- cbind(dummy[subset], dummy[response])
  corM <- cor(testedSet)
  k <- length(subset)
  
  FCC<-c()
  for (i in 1:ncol(corM)-1) {
    FCC <- c(FCC, abs(corM[response, i]))
  }
  
  FFC <- c()
  for (i in 2:(ncol(corM)-1)) {
    for (j in 1:i-1) {
      FFC <- c(FFC, abs(corM[i, j]))
    }
  }
  ...
  measure <- (k*meanFCC)/(sqrt(k+k*(k-1)*meanFFC))
  measure <- (k*meanFCC)-0.4*(sqrt(k+k*(k-1)*meanFFC))
  return(measure) 
}

FSCalculationCFS <- function() {
  
  subset <- best.first.search(attributes = features, eval.fun = evaluator, max.backtracks = 1000)
  selectedFeatures <- data.frame(ncol = ncol(features))
  
  for (i in features) {
    ifelse (i %in% subset, selectedFeatures[1, i] <- 1, selectedFeatures[1, i] <- 0) 
  }
  bestFeatures <- DeDummifyResults(selectedFeatures)
  
  FResults <- list(bestFeatures = bestFeatures)
  FResults[["Suc"]] <- Suc(FResults$bestFeatures, file)
  return(FResults)
}
...
for (f in files) {
  ...
  features <- names(dummy[,!(names(dummy) %in% c(response))])
  resultCon[[f]] <- list(bestFeatures = data.frame(), Suc = data.frame())
  for (i in 1:iter) {
    startTime <- Sys.time()
    results <- FSCalculationCFS() 
