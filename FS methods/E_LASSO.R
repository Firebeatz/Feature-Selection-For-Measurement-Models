# LASSO with glm (embedded method)
...
getDummifiedData <- function (file) {
  # kategorielle Variablen muessen fuer dummyVars noch als Faktor gecastet werden
...
# 01.1.b Aufteilung des Datasets
...
# g) LASSO Implementation with generalized linear model using glmnet from Friedman, J.; Hastie, T.; Tibshirani, R. (2010)
FSCalculationRegularization <- function (f, i) {
  
  regs <- as.matrix(trainingSet[,!names(trainingSet) %in% c(response)])
  res <- trainingSet[[response]]
  model <- cv.glmnet(regs, res)
  
  #lambda.1se: lambda which gives the most regularized model such that error is within one standard error of the minimum
  lambda <- model$lambda.1se 
  
  featureValues <- as.data.frame(t(as.matrix(coef(model, s=c(lambda)))))
  selectedFeatures <- as.data.frame(t(apply(featureValues[,-1], MARGIN = 2, FUN = function(x) ifelse(x!=0, x<-1, x<-0))))
  bestFeatures <- DeDummifyResults(selectedFeatures)
  Suc <- Suc(bestFeatures, file)
  
  results <- list(bestFeatures = bestFeatures, featureValues = featureValues, Suc=Suc)
...
for (f in files) {
  ...
  resultCon[[f]] <- list(featureValues = data.frame(), bestFeatures = data.frame(), Suc = data.frame())
  for (i in 1:iter) { 
    startTime <- Sys.time()
    DataPart <- partData(dummy)
    trainingSet <- DataPart$trainingSet
    testSet <- DataPart$testSet
    results <- FSCalculationRegularization(f, i) 
