...
#Note: Choose method = "battiti" and ratio = (0,1) for MIFS-Filter (Battiti, 1994)  or method = "Peng" for mRMR-Filter (Peng, 2005)
...
# a) returns a list object that contains the names of all relevant and all irrelevant variables of a dataset. 
...
# b) Implementation of success measure Suc, according to [BOLO-13]
...
# c) Implementation of Stability Measures, according to [NOGU-18]
...
# d) defines threshold for MIFS/mRMR results and returns a dataframe that marks which features have been selected (1) according to the threshold
...
# f) mRMR-filter implementation using varrank from Kratzer, G.; Furrer, R. (2018)
FSCalculationMI <- function() {
  results <- varrank(data.df = file, 
                     method = "peng", 
                     #ratio = 0.2, 
                     variable.important = "Response", 
                     discretization.method = "scott", 
                     algorithm = "forward", 
                     scheme="mid", 
                     verbose = FALSE)
  
  features <- names(file[-match(response, names(file))])
  scores <- c()
  for (f in features) {
    scores <- c(scores, results[[2]][f,f])
  }
  featureValues <- t(data.frame(scores))
  colnames(featureValues) <- features
  featureValues <- replace_na(featureValues, -Inf)
  bestFeatures <- selectThreshold(featureValues, 0)
  Suc <- Suc(bestFeatures, file)
  FResults <- list(featureValues=featureValues, bestFeatures=bestFeatures, Suc=Suc)
  return(FResults)
}

# SECTION 4/5: MAIN CODE (EXECUTION)
