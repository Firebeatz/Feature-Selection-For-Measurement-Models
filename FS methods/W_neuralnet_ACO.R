# ANN + Ant Colony Optimization wrapper
...
# c) normalizes data from range [0,1]
preProc <- function(train, test) {
  preProcValues <- preProcess(train, method = c("range"))
  trainTransformed <- predict(preProcValues, train)
  testTransformed <- predict(preProcValues, test) 
  FResult <- list(trainTransformed = trainTransformed, testTransformed=testTransformed)
  return(FResult)
}

# d) returns a list object that contains the names of all relevant and all irrelevant variables of a dataset. 
...
# e)  back transformation of related dummy variables into one variable again for better interpretation of results 
...
# f) Implementation of success measure Suc, according to [BOLO-13]
...
# g) Implementation of Stability Measures, according to [NOGU-18]
...
# h) Wrapper Implementation with ANN and antColony search algorithm using FSinR from 
# Aragón-Royón, F.; Jiménez-Vílchez, A.; Arauzo-Azofra, A.; Benitez, J.(2020) and caret from Kuhn, M. (2020):
FSCalculationNeuralNet <- function (f, i) {
  resamplingParams <- list(method = "cv",     
                           number = 5
                           #repeats = 3
                          )
  
  grid <- expand.grid(size = c(14, 10, 6), decay = c(0, 0.1, 0.01))
  
  fittingParams <- list(tuneGrid = grid,
                        linout = TRUE,
                        trace = FALSE
  )
  
  evaluator <- wrapperEvaluator("nnet",
                                resamplingParams,
                                fittingParams)
  
  searcher <- searchAlgorithm("antColony", list(
                              population = 20,
                              iter = 20,
                              a=1,
                              b=0.8,
  ...
  #results
  featSelection <- featureSelection(trainingSet, class= "Response", searcher=searcher, evaluator=evaluator)
...
for (f in files) {
  ...
  resultCon[[f]] <- list(bestFeatures = data.frame(), Suc = data.frame(), time = data.frame())
  for (i in 1:iter) { #schwierige Entscheidung: welche Anweisungen sollen wiederholt werden pro Datensatz, welche nicht
    DataPart <- partData(dummy)
    trainingSetOriginal <- DataPart$trainingSet
    testSetOriginal <- DataPart$testSet
    PP <- preProc(trainingSetOriginal, testSetOriginal)
    trainingSet <- PP$trainTransformed
    testSet <- PP$testTransformed
    results <- FSCalculationNeuralNet(f, i)
