...
getDummifiedData <- function (file) {
  ...
  dummyBuilder <- dummyVars(~ ., data = file, sep = "", fullRank = TRUE)
...
# g) Random Forest Implementation using randomForest from Liaw, A.; Wiener, M. (2002) and caret from Kuhn, M. (2020):
FSCalculationRandomForest <- function (f, i) {
  
  trControl <- trainControl(method="cv", 
                            number=10)
  
  grid <- expand.grid(mtry = (ncol(file)-1)/3)
  
  randomF <- train(Response ~ .,  #alternatively use X- und Y-notation instead of formula-notation
                   data = trainingSet, 
                   method="rf",
                   ntree = 1000,
                   nodesize = 3,
                   #maxnodes = as.integer(0.2*nrow(trainingSet)),
                   importance = TRUE,
                   tuneGrid = grid
  )
  
  featureImportanceModel <- as.data.frame(t(varImp(randomF, useModel = TRUE, scale = FALSE)[[1]]))
  
  # Features that are never used to split the data in the tree-building process will not be present in the featureImportanceModel at all.
  # In the following, these features will be added to the resultMatrix and set to 0 in order to get a matrix with the same number of columns in every iteration. 
  # (regardless of the features that have been selected) 
  dummyNames <- names(getDummifiedData(file[,!(names(file) %in% c(response))]))
  featureImportance <- setNames(data.frame(matrix(0, ncol=length(dummyNames), nrow=1)), dummyNames)
  for (f in dummyNames) {
    if(!(is.null(featureImportanceModel[[f]]))) {
      featureImportance[[f]] <- featureImportanceModel[[f]]
    }
  }
  
  selectedFeatures <- as.data.frame(t(apply(featureImportance, MARGIN = 2, FUN = function(x) ifelse(x>=0.05*max(featureImportance), x<-1, x<-0))))
  bestFeatures <- DeDummifyResults(selectedFeatures)
  Suc <- Suc(bestFeatures, file)
  
  results <- list(bestFeatures = bestFeatures, featureImportance = featureImportance, Suc = Suc)
...
for (f in files) {
  ...
  #dummy <- getDummifiedData(file) #Only for naming purposes in FSCalculationRandomForest()
  resultCon[[f]] <- list(featureImportance = data.frame(), bestFeatures = data.frame(), Suc = data.frame())
  for (i in 1:iter) { 
    startTime <- Sys.time()
    DataPart <- partData(file)
    trainingSet <- DataPart$trainingSet
    testSet <- DataPart$testSet
    #dummyTrainingSet <- partData(trainingSet)[["trainingSet"]]
    results <- FSCalculationRandomForest(f, i) 
    resultCon[[f]][["featureImportance"]] <- rbind(resultCon[[f]][["featureImportance"]], results$featureImportance)
