...
iter <- 5
response <- "Response"
resultCon <- list() # Listenobjekt zur Speicherung der Ergebnisse
...
Suc <- function (bestFeatures, file) {
  ...
  for (i in REL) {
    tryCatch(            #tryCatch-Block zum Abfangen von fehlenden Ausprägungsstufen wie in DS 13 (kein V06_M3)
...
# g) Wrapper Implementation with linear model and SFS search algorithm using FSinR from 
